...
// Tabs: Performance | Feed Cost | Batches | Health
...
// ─── Helper: normalise Supabase FK joins (may return array or object) ─────────
...
export default async function ReportsPage({
  ...
  if (!checkPermission(role, ACTION.MANAGE_REPORTS)) {
    redirect('/');
  }
  ...
  const [
    ...
    // 1. CONSUMPTION stock transactions → Feed Cost tab
    supabase
    ...
    // 2. Feeding records with cost + kg → daily cost trend
    supabase
      .from('feeding_records')
      .select('feeding_timestamp, total_cost, total_kg_fed')
      .eq('organization_id', orgId)
      .gte('feeding_timestamp', fromISO)
      .lte('feeding_timestamp', toISO)
      .order('feeding_timestamp', { ascending: true }),

    // 3. All animals → Performance + Batch tabs
    supabase
      .from('animals')
      .select(
        'id, tag_id, breed, status, intake_weight, intake_date, current_weight, current_adg, dispatch_ready, batch_id, pens(pen_name), batches(batch_code, arrival_date, source_supplier)'
      )
      .eq('organization_id', orgId),

    // 4. Batches lookup → Batch tab headers
    supabase
      .from('batches')
    ...
    // 6. Treatment records → Health tab
    supabase
  ...
  const rawFeedings = (feedingRecordsResult.data ?? []) as RawFeedingRecord[];
  ...
  for (const tx of rawStockTxs) {
    ...
    if (existing) {
      ...
      ingredientMap.set(tx.ingredient_id, {
        name,
        kgConsumed: tx.quantity_kg,
  ...
  const ingredientRows: FeedCostIngredientRow[] = Array.from(ingredientMap.entries()).map(
    ([ingredient_id, v]) => ({
      ...
      kg_consumed: v.kgConsumed,
      total_cost: v.totalCost,
    })
  );

  // Daily cost: group feeding_records by date
  ...
  const batchStatsMap = new Map<
    string,
    { active: number; sick: number; dead: number; dispatched: number; adgSum: number; adgCount: number }
  >();

  for (const animal of rawAnimals) {
    if (!animal.batch_id) continue;
    const s = batchStatsMap.get(animal.batch_id) ?? {
      active: 0, sick: 0, dead: 0, dispatched: 0, adgSum: 0, adgCount: 0,
    };
  ...
  const batchRows: BatchReportRow[] = rawBatches.map((b) => {
    ...
    const totalHead = (stats?.active ?? 0) + (stats?.sick ?? 0) +
      (stats?.dead ?? 0) + (stats?.dispatched ?? 0);
    return {
      batch_id: b.id,
  ...
  const exportData = [
    {
      ...
      rows: performanceRows.map((r) => ({
        ...
        Pen: r.pen_name,
        Batch: r.batch_code ?? '',
        'Days on Feed': r.days_on_feed,
        'Intake Weight (kg)': r.intake_weight,
        'Current Weight (kg)': r.current_weight,
        'ADG (kg/day)': r.current_adg,
    ...
    },
    {
      ...
      rows: ingredientRows.map((r) => ({
        ...
        'Kg Consumed': r.kg_consumed,
        'Total Cost (KES)': r.total_cost,
      })),
    },
    {
      ...
      <header className="bg-emerald-950 px-6 py-5 shadow-md print:hidden">
        <div className="max-w-6xl mx-auto">
          ...
          <p className="mt-0.5 text-sm text-emerald-200">
            Performance, feed cost, batch, and health summaries.
          </p>
        </div>
      </header>
